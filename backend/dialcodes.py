"""Country dial codes, for the prefix dropdown that sits beside every phone field.

One list, served to the browser, so the dropdown and the server can never disagree about what
"+359" means. Names are English on purpose: they sort predictably and a buyer picking a prefix
recognises their own country in English far more reliably than in a third language.
"""

# (ISO 3166-1 alpha-2, English name, dial code without the plus)
COUNTRIES = [
    ("AF", "Afghanistan", "93"), ("AL", "Albania", "355"), ("DZ", "Algeria", "213"),
    ("AS", "American Samoa", "1684"), ("AD", "Andorra", "376"), ("AO", "Angola", "244"),
    ("AI", "Anguilla", "1264"), ("AG", "Antigua and Barbuda", "1268"),
    ...
    ("CD", "Congo (DR)", "243"), ("CR", "Costa Rica", "506"), ("HR", "Croatia", "385"),
    ("CU", "Cuba", "53"), ("CW", "Curacao", "599"), ("CY", "Cyprus", "357"),
    ("CZ", "Czechia", "420"), ("DK", "Denmark", "45"), ("DJ", "Djibouti", "253"),
    ("DM", "Dominica", "1767"), ("DO", "Dominican Republic", "1809"),
    ...
    ("GF", "French Guiana", "594"), ("PF", "French Polynesia", "689"),
    ...
    ("QA", "Qatar", "974"), ("RE", "Reunion", "262"), ("RO", "Romania", "40"),
    ("RU", "Russia", "7"), ("RW", "Rwanda", "250"), ("KN", "Saint Kitts and Nevis", "1869"),
    ("LC", "Saint Lucia", "1758"), ("VC", "Saint Vincent", "1784"), ("WS", "Samoa", "685"),
    ...
    ("SB", "Solomon Islands", "677"), ("SO", "Somalia", "252"),
    ...
    ("TC", "Turks and Caicos Islands", "1649"), ("TV", "Tuvalu", "688"),
    ...
    ("VN", "Vietnam", "84"), ("VG", "Virgin Islands (British)", "1284"),
    ("VI", "Virgin Islands (US)", "1340"), ("YE", "Yemen", "967"), ("ZM", "Zambia", "260"),
    ("ZW", "Zimbabwe", "263"),
]

LIST = [{"iso": iso, "name": name, "dial": dial} for iso, name, dial in COUNTRIES]
_BY_ISO = {iso: dial for iso, _name, dial in COUNTRIES}

FALLBACK = "BG"


def dial_of(iso):
    """The dial code for a country, or Bulgaria's — this is a Bulgarian-first business."""
    return _BY_ISO.get((iso or "").upper()) or _BY_ISO[FALLBACK]
