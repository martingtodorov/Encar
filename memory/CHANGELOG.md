...
## 2026-06 — Пагинацията на телефон: цял екран, равни бутони
* `ResultsPagination.js`: редът е `w-full` на телефон, всяка клетка `flex-1`, височина 44px
  (thumb target); от `sm` нагоре пада към старата естествена ширина до формата "към страница".
* Точките (`…`) вече са `hidden sm:inline` и стоят ИЗВЪН разтягащата се клетка — на телефон
  първата/последната страница не се показват, така че многоточието само крадеше ширина и
  правеше един от бутоните тесен (24px срещу 45px).
...
## 2026-06 — Периодични 403 от Encar: диагноза и по-разумна реакция
* Проверка на живия сървър: сайтът връща 200 навсякъде (0.25–0.35s), пазачът на тунела е жив
  (`egress_ok: true`, handshake 6s, НУЛА поправки след деплоя), каталогът е цял (196 829 активни),
  а upstream отговаря 200 в момента на проверката.
* 403-ките са периодични WAF блокировки от CloudFront: 14 на 5 130 заявки (~0.3%), сами се
  изчистват за секунди. Тялото е „403 ERROR ... Request blocked" (не geo-блок).
* Проверено, че НЕ е въпрос на headers/fingerprint: с `curl_cffi` и 5 профила за имитация
  (chrome, chrome124, chrome131, safari17_0, firefox133) отговорът е пак 403. Блокировката е по
  репутация на изходния IP — не може да се заобиколи с код, а политиката на `encar.py`
  (docstring + memory/encar_api.md §8) изрично забранява ротация на адреси и заобикаляне.
* Затова е сменена НАШАТА реакция: единична блокировка вече затваря breaker-а за 25s
  (`BLOCK_COOLDOWN_FIRST`), а дългите 180s се дават само при 3 блокировки в рамките на 5 минути
  (`BLOCK_REPEAT_N`/`BLOCK_REPEAT_WINDOW`). Нула добавени повторни заявки, нула ротация.
  Дотогава един 403 от 0.3% заявки спираше ВСИЧКО за 3 минути и некешираните коли падаха
  на резервния вариант.
* `/api/health` вече става `ok:false` веднага при отворен breaker с 2+ отваряния (не чака
  10-минутната тишина), с текст кой е бил отказът и че некешираните коли се сервират от каталога.
* Тестове: ескалацията на cooldown-а проверена (25/25/180 и връщане на 25 след изтичане на
  прозореца), честността на health-а — с подправени статистики; 34 теста от свързаните suite-и
  минават. 3-те падания в `test_sync_retire_guard.py` при паралелен старт с други suite-и са
  ПРЕДИ промените (проверено с `git stash`) — изолация между тестове, не регресия.

## 2026-06 — Маршрутът към Encar е ВЕРИГА от три стъпки: директно → Mac mini → IPRoyal
* Преди: един слот `ENCAR_PROXY_URL` и `backend.env.j2` избираше IPRoyal ИЛИ Mac-а, никога и
  двете; `auto` знаеше само „директно, после единственото прокси". Верига от три стъпки беше
  невъзможна за конфигуриране.
* `backend/encar.py` преработен:
  - `TIER_ENV` — отделни променливи `ENCAR_HOME_EXIT_URL` и `ENCAR_RESIDENTIAL_PROXY_URL`;
    `ENCAR_ROUTES` задава реда (по подразбиране `direct,home_exit,residential_proxy`).
    Старият `ENCAR_PROXY_URL` СЕ ЧЕТЕ още като резидентния tier, за да не остане сървър без
    изход преди деплоя, но вече не се записва от шаблона.
  - Прекъсвач ПО TIER (`_breakers`): блокиран tier вече не спира работещия. `route()` е самата
    обхождане на веригата — първият tier, който не е спрян; ако всички са спрени, се връща
    онзи, който се отваря НАЙ-СКОРО. Няма отделна failover машинария (`auto_lean`, `lean`,
    `_failover_if_pending`, `FAILOVER_MIN_GAP` премахнати).
  - „Залепващ" cooldown: блок (403/407/511) или транспортна грешка при наличие на следващ tier
    → спиране за `AUTO_PROBE_GAP` (15 мин), за да не се стъпва пак на блокирания адрес всеки
    половин минута. Rate limit (429 с Retry-After) НЕ е залепващ — уважава се секундата, която
    Encar е казал. Последният tier запазва кратките 25s.
  - Изкачване обратно: `_probe_preferred` / `_probe_tier` проверяват по-предпочитаните tier-и
    във фонов режим (никога върху заявка на посетител) и чистят техния прекъсвач при 200.
  - `_scrub` чисти URL-ите на ВСИЧКИ tier-и; `status()` дава `chain` и `tiers[]` с отделно
    състояние; `verify()` обхожда веригата и успех през който и да е tier е успех.
* `server.py`: `POST /api/admin/encar-route` приема auto/direct/home_exit/residential_proxy
  (и стария „proxy" като синоним), с проверка дали tier-ът е настроен на този сървър.
* `watchdog.py`: проверката `proxy` мери ВСЕКИ проксиран tier поотделно (спящ Mac при работещ
  IPRoyal е warning, не тишина); `route` показва и веригата.
* `AdminEncarRoute.js`: четири бутона + подреден списък на веригата с отделно състояние на
  всеки tier („носи трафика", „спрян още N мин · причина", „всички са спрени — този тръгва
  първи").
* Деплой: `backend.env.j2` пише и трите променливи; `deploy_backend.yml` verify подава и трите
  и вече НЕ блокира релийза, когато Hetzner е 403-нат, а друг tier отговаря;
  `group_vars/all.yml.example` документира `encar_routes` и `encar_residential_proxy_url`.
* Тестове: `tests/test_encar_route.py` пренаписан за веригата (ред, липсващ tier, пренареждане
  през `ENCAR_ROUTES`, наследена променлива, пин + синоним, обхождане при мъртъв tier, блок
  който мести трафика, кратък cooldown на последния tier, изкачване обратно, всички спрени,
  status без тайни, verify през следващ tier) — 19 теста; `test_encar_proxy.py` адаптиран;
  нов autouse fixture в `tests/conftest.py` нулира маршрута и прекъсвачите между тестовете
  (състоянието е на процеса, не на клиента). 39/39 минават, `--syntax-check` чист.
* Проверено живо на preview: `/api/admin/encar-route` дава `chain: ["direct"]` (Mac и IPRoyal
  не са настроени там) и админ панелът показва веригата с причината „HTTP 403 from upstream".

## 2026-06 — Каталожният sync е умишлено бавен: два часа вместо девет минути
* Причина: 403-ките идваха на серии, точно след обхождане. ~436 заявки на пода от 1.2s е
  бурст, а бурстът е точно това, за което е направен WAF.
* `sync.py`: нов `paced_sweep(expected_requests)` — вдига `encar.min_interval` за времето на
  обхождането и го връща обратно (и при изключение). Един knob вместо sleep на шест места:
  пейсва и leaf страниците, и count пробите на бисекцията, и facet passes. Заявката,
  `bulk_write`-ът и записът на състоянието влизат В слота, не се добавят отгоре.
* `SYNC_TARGET_SECONDS` (по подразбиране 7200, чете се по време на изпълнение): 420 страници
  → ~17s на страница. Клампван между пода на клиента (1.2s) и 60s. `SYNC_TARGET_SECONDS=0`
  изключва изчакването (ползва се от тестовете), `14400` го прави четири часа.
* Приложено на `run_full_sync`, `crawl_partitioned` (нощното обхождане) и `_collect_ids`
  (facet passes). Състоянието пази `page_gap_s` и `eta_s`; watchdog проверката `sync` казва
  „върви, N/M страници, по една на 17s (умишлено бавно)", а `SYNC_STUCK_H=6` е далеч над 2ч.
* ПОСЕТИТЕЛИТЕ НЕ СЕ ЗАБАВЯТ: интерактивните заявки (някой отваря некеширана кола) минават
  през семафора и изобщо не пипат този throttle — покрито с тест.
* Тестове: нов `tests/test_sync_pacing.py` (аритметика, клампване, env override, вдигане и
  връщане на min_interval, връщане и при изключение, посетителите не се пейсват) + autouse
...
## 2026-06 — Обхождането вече наистина е 2 часа (беше 4)
* Причина за 4-те часа: `paced_sweep` беше около ВСЯКА партиция поотделно, значи всяка марка
  получаваше свой двучасов бюджет, а сумата се трупаше. Освен това фиксираният gap беше
  сметнат от leaf страници (400 за 200k), докато бисекцията реално прави ~1000 slice заявки
  + проби — още 2.5x отгоре.
* Нов `sync.Sweep`: ЕДИН бюджет за цялото обхождане. Паузата се преизчислява преди всяка
  заявка = оставащо време / оставащи заявки, като „оставащи заявки" се оценява от РЕАЛНИЯ
  напредък (индексирани коли / коли в upstream), който `publish()` подава на всеки няколко
  секунди. Оценката по брой страници е само начален seed.
* Предпазители: над бюджета → пода от 1.2s (1000 останали заявки = 20 мин, не още 2 часа);
  никога повече от 1/20 от оставащото време в една пауза (иначе първата заявка на малко
  обхождане изяждаше бюджета); таван 60s.
* `encar.py`: клиентът има `pacer` hook — `_throttle` пита него, вместо да се пипа
  `min_interval`. Интерактивните заявки (посетител отваря кола) не минават през този throttle.
* Вложените обхождания (facet pass в крawl) СПОДЕЛЯТ бюджета, не стартират втори.
* Pacer-ът се маха и при изключение (try/finally около целия цикъл по марки), иначе провалено
  обхождане оставяше всяка следваща заявка на 17 секунди.
* Тестове: 400, 1000 и 2000 реални заявки при оценка 1000 — и трите свършват в рамките на
  7200s (+1.2s) и не под 6800s; ускоряване вместо удвояване при преминат бюджет; уточняване
  на оценката в движение; споделен бюджет при влагане; махане на pacer-а при изключение.
  9/9 минават. E2E с mock transport: обхождане от 1 leaf при бюджет 12s отне 2.8s (малкият
  каталог не се пейсва изкуствено) и pacer-ът беше махнат след това.
