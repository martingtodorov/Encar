...
### Follow-up 5 — черно мигане при зуум + случайни скокове на колоната
* Мигането: зуумът се рисуваше в НОВ `<img>` със същия src → нов декод на всеки зуум и слотът
  оставаше черен за кадър-два. Вече зуумът се прилага върху СЪЩИЯ елемент (базовата снимка
  просто расте), а пълната резолюция се fade-ва отгоре, когато пристигне.
* Случайните скокове: (1) `overflow-anchor: none` на слотовете — scroll anchoring "коригираше"
  позицията, докато слотове влизат/излизат от рендиране; (2) scroll guard-ът вече не работи с
  grace прозорец, а следи посоката: докато има пръст — всичко е позволено; без пръст се допуска
  само инерция в същата посока до 2 s, всяко друго движение НАГОРЕ се връща обратно.
* Проверено: зуум → base img остава opacity 1 с трансформация (без черен кадър), overlay-ът
  се появява; wheel надолу/нагоре работи; програмен скок към 0 се връща на 1200.

### Follow-up 6 — фиксирана височина на слотовете, freeze, по-бърз пинч, PWA X бутон
* КОРЕНЪТ на "случайния скрол": слотовете се оразмеряваха по собствената снимка (RESERVE 4/3
  до зареждане, после реалното съотношение, с clamp 1:2 за скенери). Всеки слот, който сменя
  височина НАД визитора, влачи всичко под него → усеща се като скок. Сега ВСЕКИ слот е 4:3 от
  първия кадър (`object-contain`), т.е. височината на колоната е детерминирана и не се променя.
  Компромис: портретните снимки и скенираните документи са с черни полета (цели, зуумът е за
  четене). Ако потребителят предпочете реални височини, трябва размерите да идват от бекенда —
  thumb-овете са 356x200 crop и не носят истинското съотношение.
* Freeze при бърз скрол: докато колоната се движи БЪРЗО (стъпка > 60 px), не се декодира нищо —
  всеки слот показва вече кешираното thumb, а истинските файлове се връщат 200 ms след като
  скролът спре. Прозорецът е свит: IN_FLIGHT 1, WINDOW 1, AHEAD 2 (макс. 4 декодирани).
* `content-visibility: auto` е премахнат (Safari + flip на рендиране = още подскоци).
* Пинчът реагира по-рано: PINCH_START 1.15 → 1.06, SNAP 1.25 → 1.15.
* PWA: X бутонът беше под status bar-а/Dynamic Island в standalone → `margin-top:
  max(0.75rem, env(safe-area-inset-top))`. НЕ е проверено в тестова среда (изисква инсталирано
  приложение) — чака потвърждение от потребителя.
* Проверено: всички слотове с еднаква височина, `scrollHeight` не се променя докато снимките
  пристигат (6576 → 6576), 8% пинч вече мърда снимката, освобождаване под прага се връща на 1.

### Follow-up 7 — приоритет: всичко във вюпорта се зарежда
* `PhotoColumn.jsx` пренаписан около ВИДИМИЯ обхват вместо около един индекс `looking`:
  IntersectionObserver поддържа жив set от видими слотове и комитва {lo, hi} веднага (без
  debounce). Всичко на екрана се стартира моментално и подминава опашката (`fetchPriority`
  high), независимо колко други заявки са в ход; извън екрана — макс. 2 наведнъж и само докато
  колоната стои на място (AHEAD 2, BEHIND 1).
* Декодирани остават само видимите + по един встрани; при бърз flick офскрийн слотовете не се
  искат изобщо (кешираните thumb-ове покриват), заявките се връщат 200 ms след спиране.
* Проверено на 3 позиции (начало, +1000 px, след силен flick): всички видими слотове са
  `data-loaded=true`, общо декодирани 4-6.

### Follow-up 8 — намерен коренът на скока към началото + мъртвия touch
* CarDetailPage: диалогът ВЕЧЕ НЕ сменя `overflow`/`touch-action` при зуум (беше
  `photoZoom ? none/hidden : pan-y/auto`). Safari нулира позицията на скрола, когато
  overflow-ът на скролер се смени по средата на жест — точно това беше "случайният скок към
  началото" след зуум на няколко снимки, а ако флагът заседне, целият диалог отказваше всяко
  докосване. Зуумнатата снимка блокира колоната сама: нейният gesture слой покрива екрана с
  `touch-action: none`. `photoZoom` state и `onZoomChange` премахнати.
* Фантомен втори пръст с един пръст: изтекъл `pointerup` (iOS не го доставя, когато системата
  вземе жеста) оставяше стар pointerId в картата и следващият единичен пръст идваше като втори
  → пинч от няколко процента. Сега при нов жест или при пръст, от който не се е чувало повече
  от 1200 ms, картата се изчиства. Добавен и window `pointerup` като изход.
* ColumnPhoto при unmount докладва zoom off (иначе колоната мисли, че нещо е зуумнато).
...
### Follow-up 10 — ВЪРНАТО: нищо не се трие
* Започнах премахване на `Lightbox.jsx` (десктоп прегледът с релсата от миниатюри), но
  потребителят изрично каза да НЕ се трие нищо. Файлът е възстановен (`git checkout`), заедно с
  `shot` state, import-а, `openPhotos` с проверката за ≥1024px и рендирането на `<Lightbox>`.
  DialogContent е върнат на fullscreen (десктоп центрираната рамка е отменена).
* Проверено на 1920x800: десктоп прегледът се отваря както преди — релса от миниатюри,
  стрелки, zoom контрола. Мобилната колона е незасегната.

### Follow-up 11 — десктоп lightbox-ът никога не се показва на мобилно
* Проверката на ширината при отваряне не беше достатъчна (таблет, завъртян в портрет, или
  свит прозорец оставяха `Lightbox` върху тъч екран). Добавен `matchMedia("(min-width:
  1024px)")` listener в CarDetailPage: при излизане от wide режим `shot` се затваря веднага.
* Проверено: на 1920 се отваря; при свиване до 430 изчезва сам; тап на телефон отваря
  вертикалната колона, не стария преглед.

### Follow-up 12 — native lazy load като втора линия на защита
* `ColumnPhoto.jsx`: `loading={priority ? "eager" : "lazy"}` за пълните снимки (видимите —
  eager + `fetchPriority: high`, останалите — lazy) и `loading="lazy"` за thumb слоя. Заедно с
  прозореца от PhotoColumn това значи, че в DOM има макс. ~5-6 пълни снимки, а браузърът сам
...
### Follow-up 15 — "Автоматично" вече значи ДИРЕКТНО с прокси само при отказ
* `encar.py`: `auto` вече не значи "прокси, ако е налично". Директният маршрут е нормалният;
  проксито е патерица (`_auto["on_proxy"]`), която се вдига само след ТРАНСПОРТЕН отказ
  (403/407/429 не мърдат маршрута — това е отказ от Encar, не счупен хоп).
  - `proxy_url()` в auto връща прокси само докато `on_proxy`; нов `auto_lean()`, `auto_on_proxy()`.
  - `other_route()` в auto: "proxy" когато сме директно, "direct" когато сме на проксито.
  - `_failover_if_pending()` в auto НЕ сменя режима (`switch_route`), а само патерицата
    (`EncarClient.lean`), за да остане в сила инструкцията "директно, ако може".
  - Нов `_probe_direct()` + `_maybe_probe_direct()`: на всеки `AUTO_PROBE_GAP = 900 s` (15 мин)
    ФОНОВА заявка към `PROBE_PATH` (/v1/readside/vehicles/car/options/standard) без прокси; при
    200 патерицата пада и трафикът се връща директно (записва се през `_persist`). Заявката на
    посетителя никога не чака проверката.
  - `set_route(mode)` изчиства патерицата — избирането на "Автоматично" в админа е и "пробвай
    директното сега".
  - `status()` дава `auto_on_proxy`, `auto_since`, `probe_in_s`, `last_probe`.
  - Bugfix: `_scrub()` вече чете КОНФИГУРИРАНОТО прокси от env, не активното — иначе в auto
    режим credentials-ите не се маскираха в логовете.
* Админ UI (`AdminEncarRoute.js`): подсказката на "Автоматично" е "директно, прокси при отказ";
  нов ред `admin-encar-route-lean`, който казва че сме временно на прокси и след колко минути
  се проверява директното.
* Тестове: `tests/test_encar_route.py` пренаписан за новата семантика + 4 нови теста (патерица
  надолу/нагоре, probe връща на директно, probe не пипа при още счупено директно, probe не се
  трупа). `tests/test_encar_proxy.py`: двата теста за прокси вече избират режим "proxy" явно.
  31 passed. Провалите в `test_sync_retire_guard.py` при комбиниран xdist пуск СЪЩЕСТВУВАТ и
  без тази промяна (проверено със stash) — не са регресия.
* Живо: `GET /api/admin/encar-route` → mode=auto, route=direct.

### Follow-up 16 — НАМЕРЕНА истинската причина за freeze-а: CDN игнорираше resize-а
* `encar.py / full_image_url()`: `impolicy=widthRate&rw=N` БЕЗ crop височина се игнорира
  тихо — CDN връщаше суровия оригинал 2200x1238 (1.2 MB на кабела, ~11 MB bitmap след декод)
  за КАЖДА снимка в колоната, макар URL-ът да казваше rw=640. Затова iPhone-ът блокираше.
  Емпирично: `rw=640&ch=2560` → 640x360, 24 KB; `ch` НА или ПОД скалираната височина
  (напр. ch=360 при rw=640) пак се игнорира — оттам щедрият множител `ch = rw * 4`.
  Проверено на 5 различни обяви: 640x360, 24-46 KB.
* `full_column` = 640 (беше 900, но реално 2200), `full_lightbox` = 1600 (реално 2200).
* ВСИЧКИ снимки на Encar са 16:9 (2200x1238 или 2040x1148) — няма портретни източници. Затова
  `RESERVE` в PhotoColumn е 16/9: снимката запълва слота точно, нула черни полета, и пак нула
  layout shift.
* Зареждане по РАЗСТОЯНИЕ, не по брой слотове: втори IntersectionObserver с
  `rootMargin: 400px` дава "почти на екрана"; само тези се стартират след видимите, и то
...
## Скорост на обхождането (обновено 2026-06)
Пълното обхождане на каталога е умишлено разтеглено на ~2 часа (`SYNC_TARGET_SECONDS=7200`,
чете се по време на изпълнение), ~17s на страница при ~420 страници. Механизъм:
`sync.paced_sweep()` вдига `encar.min_interval` за времето на обхождането и го връща обратно —
пейсва leaf страници, count проби и facet passes наведнъж. Посетителите НЕ се забавят
(интерактивните заявки минават през семафора, не през throttle-а). Нощното разписание е 03:30
веднъж дневно, така че двучасов sweep не се застъпва; `SYNC_STUCK_H=6` не алармира.
